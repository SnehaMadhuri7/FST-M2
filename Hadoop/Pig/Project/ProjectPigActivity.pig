-- Load Data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/sneha/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/sneha/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/sneha/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/sneha/outputs;

-- Store Result in HDFS
STORE nameOrdered INTO 'hdfs:///user/sneha/outputs' USING PigStorage('\t');